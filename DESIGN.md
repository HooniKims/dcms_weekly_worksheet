...
### Paperlogy — Display and headline face · `--font-signifier`
- **Source:** The same self-hosted Paperlogy files as the UI token
- **Fallback:** Pretendard, Noto Sans KR, or ui-sans-serif/system-ui stack
- **Weights:** 400, 500, 700
- **Sizes:** 44px, 64px, 90px
- **Line height:** 1.30
- **Letter spacing:** -2.25px at 90px, -0.96px at 64px, -0.66px at 44px
- **Role:** Application display and heading text. The weekly report remains Malgun Gothic.

### Paperlogy — Body, UI, and navigation sans · `--font-sohne`
- **Source:** [Freesentation/Paperlogy](https://github.com/Freesentation/paperlogy), self-hosted WOFF2 files
- **Fallback:** Pretendard, Noto Sans KR, or ui-sans-serif/system-ui stack
- **Weights:** 400, 500, 700
- **Sizes:** 14px, 15px, 16px, 17px, 18px, 20px, 22px, 26px
- **Line height:** 1.00–1.50
- **Letter spacing:** -0.234px at 26px, -0.162px at 18px, 0 at body sizes
- **Role:** Every application screen, navigation, field, and dialog. The printable weekly report is explicitly excluded.
...
| lock-heading-mobile | 32px | 1.18 | -0.035em | `--text-lock-heading-mobile` |
...
:root {
  ...
  --font-signifier: 'Paperlogy', Pretendard, 'Noto Sans KR', ui-sans-serif, system-ui, sans-serif;
  --font-sohne: 'Paperlogy', Pretendard, 'Noto Sans KR', ui-sans-serif, system-ui, sans-serif;
...
## School identity asset contract

- The supplied Deungchon Middle School emblem is the identity source. The production master lives at
  `/public/deungchon-logo.png` as a 478px square RGBA PNG with a circular transparent exterior. Its
  original 478×475 artwork is not stretched; only transparent vertical padding completes the square.
- The lock screen displays the emblem before the `WEEKLY WORK` eyebrow at 104px on desktop and 80px
  below 860px. At the same breakpoint the lock screen stacks into one column so Korean display text
  keeps natural phrase boundaries. The emblem keeps its native colors with no shadow or recoloring.
- `/public/favicon.png` is the 64px browser icon and `/public/apple-touch-icon.png` is the 180px touch
  icon. Both derive from the same transparent master so the mark remains round on browser surfaces.

## Google Sheets report fidelity contract

The workspace chrome follows the Steep design system above. The weekly report itself is a
...
- Typeface: `Malgun Gothic` throughout the report. The screen document keeps the 13pt title,
  11pt metadata, 9pt body, and 8pt embedded-table scale. Print output enlarges these together to
  13.54pt, 11.45pt, 9.37pt, and 8.33pt respectively.
- Header: row 1 is a 22px centered title; row 2 is an 18px metadata row with
  `회의종류 :: 교무회의` left and the Korean date including weekday right; row 3 is a 1px spacer.
- Department cells: centered bold label, `#d7c2e5` fill, `#8d8d8d` 1px borders. Content cells are
  white, left aligned, vertically centered, and use the same outer border color.
- Embedded tables: centered 8pt content with `#8f9bad` borders; header cells use `#eef2f8` and bold
  text. Bold, italic, underline, lists, links, row span, and column span remain visible in screen and
  PDF output.
- The document has no rounded card edge, drop shadow, oversized serif heading, or empty-state copy.
  Empty departments render the same compact `* 없음` row used by the source sheet.
- The initial `말씀 및 기도` department is marked with the semantic `omitWhenEmpty` flag. When that
  flagged department has no content, its complete row is omitted from screen and print/PDF even
  after an administrator renames it. It remains selectable in the entry workspace.

## Workspace date selector contract

- The most recent week is selected on entry and displayed as a single `작성할 날짜` selector with
  the full year, month, day, weekday, and `현재 입력` status.
- Historical weeks are revealed as native selector options only after interaction. Selecting one
  loads that week's saved department entries through the repository, matching the previous Apps
  Script experience without rendering a long week-button list.

## Workspace session and week-snapshot contract

- The lock-screen hero reads `등촌중학교 주간업무 추진사항 수합 사이트`. The access card does not
  repeat that title; it begins with the `내부 업무 공간` label and password guidance. The browser title
  remains `등촌중학교 주간업무추진사항 수합 사이트`. Long Korean titles may wrap only at natural
  word boundaries.
- The workspace opens in `편집` view. The report is an explicit secondary tab so contributors land
  directly in the department editor rather than a PDF-like preview.
- A contributor session never renders the compiled `주간표` tab or `A4 인쇄` action. Those controls
...
- Department labels and selection are scoped to the selected week's `departmentSnapshot`. The active
  master department list is never used to redraw a historical week, preserving that week’s names,
  order, and membership.
- On date change, retain the current department when that department ID exists in the target week;
  otherwise select the target snapshot’s first department. The entry draft is always reloaded for the
  resulting week-and-department pair.

## Administrator department-management contract

- Administrator tools show the currently selected week in context and create weeks only when an
  administrator explicitly chooses a valid date. There is no scheduled or automatic-week messaging
  in the interface.
- After administrator authentication, the department manager edits that selected week's ordered,
  active snapshot. Rows support rename, add, move, and remove. Save rejects blank names, normalized
  duplicate names, and an empty department list before any repository request is made.
- A saved list is normalized to contiguous `order` values, with active rows marked `active: true`.
  Removing a row is a soft removal at the persistence boundary: entries and historical snapshots are
  preserved, while the selected week's new snapshot and the master list no longer expose that row.
- The save response refreshes the selected week's snapshot and retains the selected department ID
  when present; otherwise the first active department becomes the editor selection. A late save is
  ignored if the contributor has since moved to a different week or department.
...
- On desktop, the workspace header and two-column workspace grid share the same centered width:
  `min(100%, 1480px)`. The navigation track remains 280px wide with a 20px gutter; the editor
  track takes the remaining width. Switching dates, departments, or content lengths must never
  move either editor edge.
- The workspace main area reserves its vertical scrollbar gutter and every editor, search, and
  administrator surface can shrink within its containing track. This prevents a long result,
  rich-text table, or department name from producing page-level horizontal scroll. The report
  keeps its own existing overflow behavior rather than widening the application page.
- Below 860px the desktop navigation is replaced by the two compact selectors. The content area,
  search field, dialog, editor, and action row occupy the available width without horizontal
  overflow. Administrator, logout, and A4 print actions all remain visible and wrap as a group at
  very narrow widths.
...
- Archive search is available to every unlocked contributor. It waits for at least two characters,
  ...
  geometry. Selecting one opens a reuse dialog without moving away from the current week or department.
- The reuse dialog previews the saved, sanitized rich content with its source date and department. It
  targets the most recent `현재 입력` week even when the contributor searched while viewing an archive.
  The closest current department name is selected and marked as recommended, while contributors may
  choose any other current department before applying the content.
- Contributors explicitly choose `현재 주차에 추가`, `현재 주차 내용 교체`, or `원본 주차 보기`.
  Append preserves formatting on both blocks with one paragraph boundary; replace swaps only the target
  department draft; both open that current-week department, mark it dirty, and require the normal save
  action. Moving to the source keeps the original archive-navigation behavior and does not copy content.
- The reuse dialog uses the existing modal surface and a `680px` maximum-width token so embedded tables
  remain readable; its preview owns horizontal scrolling rather than widening the page.
- Search controls use the existing Paperlogy/Steep input, hairline, pill, and peach focus tokens.
  Keyboard focus is visibly preserved for the query field and each result; loading, empty, and
  error states remain textually announced.
